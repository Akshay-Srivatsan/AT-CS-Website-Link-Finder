...
# Distance = 3; Uses tardis, memory-alpha, and memory-beta wikias; < 1 minute.
origin = 'http://tardis.wikia.com/wiki/Star_Trek'
target = 'http://memory-beta.wikia.com/wiki/Borg'

# Distance = 4; ~4 minutes
# origin = 'http://la.wikipedia.org/wiki/Vicipaedia:Salve'
# target = 'http://la.wikipedia.org/wiki/Compendium'

# Distance = 3; ~4 minutes
# origin = 'http://en.wikipedia.org/wiki/Doctor_(Doctor_Who)'
# target = 'http://en.wikipedia.org/wiki/James_T._Kirk'

# origin = 'http://en.wikipedia.org/wiki/Menlo_School'
# target = 'http://en.wikipedia.org/wiki/Redwood_City_(Caltrain_station)'
        ...
        if source[i] == '<':
            ...
            if not in_script:
                if tag_type == 'a':
                    ...
                    part_of_body = 'subnav' not in tag and (True or 'title' in tag) and 'accesskey' not in tag
...
def scan_url(url, history):
    ...
    if count_urls_scanned.value % 50 == 0:
        print '*', count_urls_scanned.value, 'urls scanned'
...
count = 1
pool = ThreadPool(16)
while target_distance == float('inf') and len(scanned_urls) < 100000 and count <= 10:
    print 'starting level', count
    print 'scanning', len(current_urls), 'urls'

    count_urls_scanned.value = 0
    threads = []
    # print current_urls
    ...
    def helper(x, y):
        global __count
        orginal_list_of_urls = x[0]
        original_distance_path = x[1]
        __count += 1
        if __count % 50 == 0:
            print '-', __count, 'urls searched'
        return orginal_list_of_urls + y['urls'], (original_distance_path if original_distance_path[0] < y['distance'] else (y['distance'], y['path']))
...
if distance == float('inf'):
    print 'Couldn\'t find a connection.'
    sys.exit()
